...
LABEL_SUFFIX=""
if [ -n "${VAULT_INSTANCE_LABEL}" ]; then
  LABEL_SUFFIX="-${VAULT_INSTANCE_LABEL}"
fi

# Helm release name and pod name
export VAULT_RELEASE_NAME="${VAULT_RELEASE_NAME:-vault${LABEL_SUFFIX}}"
export VAULT_POD_NAME="${VAULT_POD_NAME:-${VAULT_RELEASE_NAME}-0}"

# Kubernetes namespace for Vault
export VAULT_NAMESPACE="${VAULT_NAMESPACE:-vault-kms${LABEL_SUFFIX}}"

# Vault Enterprise namespace (different from Kubernetes namespace)
export VAULT_ENTERPRISE_NS="${VAULT_ENTERPRISE_NS:-admin}"

# Transit key name
export VAULT_KMS_KEY_NAME="${VAULT_KMS_KEY_NAME:-kms-key${LABEL_SUFFIX}}"
...
echo "Instance Label: ${VAULT_INSTANCE_LABEL}"
echo "Release Name: ${VAULT_RELEASE_NAME}"
echo "Pod Name: ${VAULT_POD_NAME}"
...
oc wait --for=condition=ready pod/"${VAULT_POD_NAME}" -n "${VAULT_NAMESPACE}" --timeout=5m
...
oc exec "${VAULT_POD_NAME}" -n "${VAULT_NAMESPACE}" -- cat /var/run/tls/vault-ca.pem > "${CA_CERT_TMP}"
echo "  ✓ CA certificate extracted"

# Create or update ConfigMap with CA certificate in openshift-config
echo ""
echo "Creating ConfigMap vault-ca-bundle${LABEL_SUFFIX} in openshift-config..."
oc create configmap "vault-ca-bundle${LABEL_SUFFIX}" \
  --from-file=ca-bundle.crt="${CA_CERT_TMP}" \
  -n openshift-config \
  --dry-run=client -o yaml | oc apply -f -
echo "  ✓ ConfigMap vault-ca-bundle${LABEL_SUFFIX} created/updated"
...
oc exec "${VAULT_POD_NAME}" -n "${VAULT_NAMESPACE}" -- \
...
oc create secret generic "vault-approle-secret${LABEL_SUFFIX}" \
  -n openshift-config \
  --from-literal=role-id="${ROLE_ID}" \
  --from-literal=secret-id="${SECRET_ID}"
